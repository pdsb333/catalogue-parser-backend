...
import type { UserUpdate, UserOut } from "@/app/utils/schema.ui";
...
export default function UserEditForm({ initialData, onClose, onSubmit }: UserEditFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setPassword(""); // On vide le mot de passe initial
    }
  }, [initialData]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-text/60 backdrop-blur-sm p-m">
      <div className="card-fr w-full max-w-md border-t-4 border-t-fr-blue bg-bg p-l shadow-card animate-in fade-in zoom-in duration-200" role="dialog">
        <div className="flex items-center justify-between mb-m text-text">
          <h2 className="font-main text-xl font-bold">Modifier l'utilisateur</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-m mb-l">
          <div className="flex flex-col gap-xs">
            <label className="text-xs font-bold uppercase text-text-muted">Email (optionnel)</label>
            <input
              type="email"
          ...
          <div className="flex flex-col gap-xs">
            <label className="text-xs font-bold uppercase text-text-muted">Nouveau mot de passe (optionnel)</label>
            <input
              type="password"
          ...
          <div className="flex justify-end gap-m pt-m border-t border-bg-alt">
            ...
            <button type="submit" className="btn-primary px-m py-s shadow-sm cursor-pointer">Mettre à jour</button>
          </div>
        </form>
      </div>
    </div>
